...
async def index(request: Request):
    try:
        trending_task = get_trending()
        top_rated_task = get_top_rated()
        now_playing_task = get_now_playing()
        upcoming_task = get_upcoming()
        ...
        return templates.TemplateResponse(request, "index.html", {"trending": trending[:10],
        ...
        return templates.TemplateResponse(request, "index.html", {"trending": [], "top_rated": [], "now_playing": [], "upcoming": [],
            "trending_searches": []})
...
async def api_movie(tmdb_id: int, media_type: str = "movie"):
    movie = await get_movie_details(tmdb_id, media_type)
    if not movie:
